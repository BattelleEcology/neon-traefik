...
1. Rebase onto next branch version, ensure branch is referencing the same commit as version's tag
    ...
    # Fetch from upstream, checkout latest upstream version branch
    git checkout v3.7
    # Rebase from upstream if already exists
    ...
    git log
    # Switch back to neon branch
    git checkout v3.7-neon
...
6. Update .neon/Dockerfile from current release version of Dockerfile
    - https://github.com/traefik/traefik-library-image/blob/v3.7.13/v3.7/alpine/Dockerfile
