    ...
    <q-scroll-area v-if="data && data.length" :thumb-style="appThumbStyle" style="height:100%;">
      <div v-for="(middleware, index) in data" :key="index">
        ...
        <q-card-section v-if="protocol === 'http'">
          ...
          <!-- EXTRA FIELDS FROM MIDDLEWARES - [compress] - compress -->
          <q-card-section v-if="exData(middleware).acceptedContentTypes">
            <div class="row items-start no-wrap">
              <div class="col">
                <div class="text-subtitle2">Accepted Content Types Response</div>
                <q-chip
                  v-for="(range, key) in exData(middleware).acceptedContentTypes" :key="key"
                  dense
                  class="app-chip app-chip-green">
                  {{ range }}
                </q-chip>
